...
│  ────────────────────────────────────────────────────────────────────    │
│                                                                          │
│ ▌  ╭───╮  Heading One                   Body text on the right side     │ ← active row
│    │ 1 │                                (can wrap to multiple lines)     │
│    ╰───╯                                                                 │
│    ─────────────────────────────────────────────────────────────────     │
│    ╭───╮  Heading Two                   More info text here              │
│    │ 2 │                                                                 │
│    ╰───╯                                                                 │
│    ─────────────────────────────────────────────────────────────────     │
│    ╭───╮  Heading Three                                                  │
│    │ 3 │  (no body defined — col 3 hidden entirely)                      │
│    ╰───╯                                                                 │
└──────────────────────────────────────────────────────────────────────────┘
  ╰────╴  ╰────────────────────────────╴  ╰────────────────────────────────╴
  col 1         col 2                              col 3
...
Row height is _dynamic_ — the renderer estimates how many lines each column needs
...
|-------|------|---------|-------------|
| `content.highlight` | int | — | 0-based index of the row to highlight with the accent bar |
| `content.footer` | string | — | Footer text rendered at the bottom of the card |
| `title` | string | — | Card header title |
| `subtitle` | string | — | Subtitle text below the header line |
| `icon.*` | dict | — | Title icon — same fields as all other cards (`name`, `visible`, `position`, `color`) |
...
content:
  rows:
    - col1: "01"               # indicator text, icon ligature, or emoji
      heading: "Step One"
      body: "Describe the step in detail — can span multiple lines."
    - col1: "02"
      heading: "Step Two"
      body: ""                 # empty → no body rendered for this row
...
- `card_numbered_text_highlight_bar_visible` — `true` | `false`
- `card_numbered_text_highlight_bar_color` — bar fill colour
- `card_numbered_text_highlight_bar_width` — thickness in px
- `card_numbered_text_highlight_bar_gap` — gap between bar and col 1 in px

**Active row text:**
- `card_numbered_text_active_col1_font_color` — col 1 text on active row
- `card_numbered_text_active_badge_color` — badge fill on active row
- `card_numbered_text_active_heading_font_color` — col 2 heading on active row
- `card_numbered_text_active_heading_font_weight` — e.g. `700`
- `card_numbered_text_active_body_font_color` — col 3 body on active row
- `card_numbered_text_active_font_style` — `normal` | `italic`

**Column 1 — indicator:**
- `card_numbered_text_col1_width_pct` — column width as % of usable row width
- `card_numbered_text_col1_alignment` — `left` | `center` | `right`
- `card_numbered_text_col1_font_size` — px
- `card_numbered_text_col1_font_color` — used when `badge_shape` is `none`
- `card_numbered_text_col1_font_weight`
- `card_numbered_text_col1_font_style` — `normal` | `italic`
- `card_numbered_text_col1_font_family` — set to icon font name for Material Symbols ligatures

**Column 1 badge:**
- `card_numbered_text_badge_shape` — `circle` | `square` | `rectangle` | `none`
- `card_numbered_text_badge_color` — background fill colour
- `card_numbered_text_badge_size` — diameter / side length in px (circle / square)
- `card_numbered_text_badge_on_color` — text / icon colour drawn on the badge
...
**Row geometry:**
- `card_numbered_text_row_vertical_alignment` — `top` | `middle` | `bottom`
- `card_numbered_text_row_min_height` — minimum row height in px (0 = auto)
- `card_numbered_text_row_padding` — vertical padding inside each row in px
- `card_numbered_text_heading_body_gap` — gap between heading and body (future stacked mode) in px

**Row separators:**
- `card_numbered_text_separator_visible` — `true` | `false`
- `card_numbered_text_separator_color`
- `card_numbered_text_separator_width` — px
- `card_numbered_text_separator_inset` — px inset from each edge (0 = full width)

**Outer spacing:**
- `card_numbered_text_gap_top` — extra space above first row in px
- `card_numbered_text_gap_bottom` — extra space below last row in px
